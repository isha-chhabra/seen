...
export function buildReportPrompt(a: ReportPromptArgs): string {
	...
	return [
		...
		`- Use ONLY the data in the digest below. Every sentence must contain a specific number, a named buying question, a named site, or a named competitor from the digest. Delete any sentence that does not.`,
		`- No filler openers ("It is important to note", "Overall", "In summary", "As you can see").`,
		`- Never make the same point twice.`,
		`- Every action-plan item names a specific site or a specific buying question and is doable within a month. Order them by impact.`,
		`- The headline states the single most important fact of the period with its number.`,
